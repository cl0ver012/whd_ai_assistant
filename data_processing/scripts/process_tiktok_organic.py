"""
TikTok Organic Data Processing - All Properties Preserved
...
class TikTokOrganicProcessor:
    """
    Process TikTok Organic CSV files with all properties as structured columns
    ...
    def extract_period_from_filename(self, file_name: str) -> tuple:
        ...
        Format: TikTok Apr 25.csv, TikTok Dec 24.csv
        """
        month_names = {
            ...
            'July': ('07', 'July'), 'June': ('06', 'June')
        }
        
        try:
            # Extract month and year from filename
            # Pattern: "TikTok Apr 25.csv" or "TikTok Dec 24.csv"
            parts = file_name.replace('TikTok ', '').replace('.csv', '').split()
            ...
            if len(parts) == 2:
                ...
                if month_abbr in month_names:
                    ...
                    year = f"20{year_short}"  # Convert 24 -> 2024, 25 -> 2025
    ...
    def extract_day_from_date(self, date_str: str) -> str:
        """Extract day number from date string like '1 April' or '15 December'"""
        try:
            parts = date_str.split()
            if len(parts) >= 1:
                return parts[0]  # Return the day number
        except:
            pass
        return None
    ...
    def process_row(self, row: pd.Series, file_name: str, year: str, month: str, month_name: str, period: str) -> Dict:
        ...
        day_of_month = self.extract_day_from_date(date)
        
        # Engagement metrics
        video_views = self.clean_numeric_value(row.get('Video Views', 0))
        profile_views = self.clean_numeric_value(row.get('Profile Views', 0))
        ...
        has_views = (video_views or 0) > 0 or (profile_views or 0) > 0
...
Engagement Metrics:
- Video Views: {int(video_views) if video_views else 0:,}
- Profile Views: {int(profile_views) if profile_views else 0:,}
...
"""
    ...
    def get_existing_records(self, file_name: str, date: str) -> set:
        """Get all existing records for a file and date to avoid duplicates"""
        try:
            ...
            # Create a set of (file_name, date) tuples for fast lookup
            existing = {(row['file_name'], row['date']) for row in result.data}
    ...
    def store_batch(self, batch: list) -> tuple:
        ...
        try:
            result = supabase.table('tiktok_organic').insert(batch).execute()
            return len(batch), 0
        except Exception as e:
    ...
    def process_file(self, file_path: Path, batch_size: int = 100):
        """Process TikTok Organic file with batch inserts for speed"""
        print(f"\n{'='*60}")
        ...
        existing_records = self.get_existing_records(file_path.name, None)
        ...
        for idx, row in df.iterrows():
            ...
            record_key = (data['file_name'], data['date'])
    ...
    def clear_all_data(self):
        """Delete all existing TikTok Organic data from database in batches"""
        try:
            print("\n⚠️  Clearing all existing TikTok Organic data...")
            
            # Count total records first
            count_result = supabase.table('tiktok_organic').select('id', count='exact').limit(1).execute()
            ...
            while True:
                ...
                # Delete this batch
                supabase.table('tiktok_organic').delete().in_('id', ids).execute()
    ...
    def process_all_files(self, clear_existing: bool = False):
        ...
        files = sorted(self.data_folder.glob("TikTok*.csv"))
        
        print(f"\nFound {len(files)} TikTok Organic files")
        ...
        # Process files
        print("\n" + "="*70)
        print("PROCESSING TIKTOK ORGANIC FILES")
        print("="*70)
...
def main():
    ...
    print("="*70)
    print("TikTok Organic Data Processing - FAST MODE")
    ...
    # Set data folder
    data_folder = "NBX/TikTok Organic"
    ...
    # Process
    processor = TikTokOrganicProcessor(data_folder)
    processor.process_all_files(clear_existing=clear_existing)
    
    print("\n✅ All done! Your TikTok Organic data is stored with all properties preserved.")
